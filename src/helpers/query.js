module.exports = {
  getQueryWhere: (table, column_needed, column_where, value_where) => {
	...
	queryCount: (extraJoin = '', where = '', extra) => {
		return `SELECT p.product_id AS total_data FROM products AS p JOIN categories AS c ON c.category_id = p.category_id JOIN brands AS b ON b.brand_id = p.brand_id  ${extraJoin} ${where} ${extra}`
	},
}